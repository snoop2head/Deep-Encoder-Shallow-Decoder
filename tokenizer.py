...
if __name__ == "__main__":
    if CFG.TRAIN_TOKENIZER:
        ...
        korean_trainer = WordPieceTrainer(
            vocab_size=10000, special_tokens=SPECIAL_TOKENS
        )
        ...
        english_trainer = WordPieceTrainer(
            vocab_size=10000, special_tokens=SPECIAL_TOKENS
        )
        ...
        korean_tokenizer.push_to_hub(
            "PoolC/deep-encoder-shallow-decoder-ko", use_temp_dir=True
        )
        english_tokenizer.push_to_hub(
            "PoolC/deep-encoder-shallow-decoder-en", use_temp_dir=True
        )

    if not CFG.TRAIN_TOKENIZER:
        """fetch tokenizer from huggingface hub"""
        korean_tokenizer = AutoTokenizer.from_pretrained(
            "PoolC/deep-encoder-shallow-decoder-ko"
        )
        english_tokenizer = AutoTokenizer.from_pretrained(
            "PoolC/deep-encoder-shallow-decoder-en"
        )
