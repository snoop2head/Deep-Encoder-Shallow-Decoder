...
class PositionalEncoding(nn.Module):
    def __init__(
        ...
        maxlen: int = CFG.max_token_length,
    ):
...
class Seq2SeqTransformer(nn.Module):
    def __init__(
        ...
        emb_size: int = 512,
        nhead: int = 8,
        src_vocab_size: int = korean_tokenizer.vocab_size,
        tgt_vocab_size: int = english_tokenizer.vocab_size,
        dim_feedforward: int = 2048,
        dropout: float = 0.1,
    ):
        super(Seq2SeqTransformer, self).__init__()

        # Q. multihead is 8, but why does it become 2048 not 4096?
        # A. it is not concating but increasing dimension from 512 to 2048 and then
        # A. reducing to 2048 to 512 dimension
